...
import { 
  ...
  Shield, BookOpen, Users, Moon, Sun, Globe, Loader, Rss, Sparkles,
  UserCheck, PlusCircle, LogOut, BarChart
} from 'lucide-react';
...
const translations = {
  en: {
    ...
    // Gemini Feature
    organizeList: "✨ Organize with AI",
    organizing: "Organizing your packages...",
    categories: "Suggested Categories",
    geminiError: "The AI is a bit busy. Please try again in a moment.",
    // Package Item
  ...
  es: {
    ...
    // Gemini Feature
    organizeList: "✨ Organizar con IA",
    organizing: "Organizando tus paquetes...",
    categories: "Categorías Sugeridas",
    geminiError: "La IA está un poco ocupada. Por favor, inténtelo de nuevo en un momento.",
    // Package Item
  ...
  fr: {
    ...
    // Gemini Feature
    organizeList: "✨ Organiser avec l'IA",
    organizing: "Organisation de vos colis...",
    categories: "Catégories Sugerérées",
    geminiError: "L'IA est un peu occupée. Veuillez réessayer dans un instant.",
    // Package Item
...
const statuses = ['In Transit', 'Out for Delivery', 'Problem'];
...
const PackageInputForm = ({ onAddPackage, t }) => {
  ...
  const handleTrackClick = (e) => {
    ...
    if (virtualPackage) {
      // Found in virtual DB
      detectedPackage = {
        trackingNumber: virtualPackage.trackingNumber,
        carrier: virtualPackage.carrier,
        status: virtualPackage.status,
        estimatedDelivery: virtualPackage.estimatedDelivery,
        name: virtualPackage.name // Pre-fill name from virtual DB
      };
...
const GeminiOrganizeModal = ({ isOpen, onClose, isLoading, categories, error, t }) => (
  <Modal isOpen={isOpen} onClose={onClose} title={t('categories')}>
    {isLoading && (
      <div className="flex flex-col items-center justify-center h-48">
        <Loader className="w-10 h-10 text-blue-600 animate-spin" />
        <p className="text-gray-600 dark:text-gray-400 mt-4">{t('organizing')}</p>
      </div>
    )}
    {error && (
      <div className="flex flex-col items-center justify-center h-48 text-center">
        <AlertTriangle className="w-10 h-10 text-red-500 mb-4" />
        <p className="text-red-600 dark:text-red-400 font-medium">{t('geminiError')}</p>
      </div>
    )}
    {!isLoading && !error && categories && (
      <div className="space-y-5">
        {Object.entries(categories).map(([category, items]) => (
          <div key={category}>
            <h4 className="text-lg font-semibold text-blue-700 dark:text-blue-400 mb-2">{category}</h4>
  ...
  <PageContent title="Frequently Asked Questions">
    <div className="space-y-6">
      ...
      <div>
       ...
       <div>
        <h3 className="font-semibold text-xl">What does "✨ Organize with AI" do?</h3>
        <p>This feature is powered by the Gemini AI. It reads your list of *package names* (e.g., "New Running Shoes", "Birthday Gift") and sends them to the AI to be sorted into logical categories. This does not affect your list, it just shows you a "smart" summary.</p>
      </div>
    </div>
  </PageContent>
);
...
const PrivacyPage = () => (
  <PageContent title="Privacy Policy">
    ...
    <h3 className="font-semibold text-xl mt-4">4. Gemini AI Feature</h3>
    <p>The "Organize with AI" feature sends *only* your list of package names (e.g., "New Running Shoes") to the Google Gemini API for processing. This data is not stored or reviewed and is only used to generate the categories. Your tracking numbers are *never* sent.</p>
  </PageContent>
);
...
const DashboardContent = ({ packages, handlers, t, language }) => {
  ...
  const organizeMenuRef = useRef(null);
  useOnClickOutside(organizeMenuRef, () => setIsOrganizeMenuOpen(false));
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="relative" ref={organizeMenuRef}>
            <button
              ...
              disabled={packages.filter(p => !p.isArchived && p.status !== 'Delivered').length < 2} 
              className="p-2 rounded-full text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700 disabled:opacity-50 disabled:cursor-not-allowed"
              aria-label="Organize options"
            >
              <MoreVertical className="w-5 h-5" />
            </button>
            
            {isOrganizeMenuOpen && (
              <div className="absolute right-0 top-10 w-56 bg-white dark:bg-gray-700 rounded-md shadow-lg border dark:border-gray-600 z-20 animate-fade-in-fast">
                <button
...
const getGeminiOrganization = async (packageNames) => {
  const apiKey = ""; // API key is handled by the environment
  const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-preview-09-2025:generateContent?key=${apiKey}`;

  const systemPrompt = `You are a smart delivery organizer. Given a JSON list of package names, group them into logical categories (e.g., "Clothing", "Gifts", "Electronics", "Household"). 
Respond ONLY with JSON in the format { "Category Name": ["package name 1", "package name 2"] }. 
If a package doesn't fit a category, put it in "Miscellaneous". Ensure every package name from the input list is assigned to exactly one category. Only use package names from the input list.`;

  const payload = {
    contents: [{
      parts: [{ text: JSON.stringify(packageNames) }]
    }],
    systemInstruction: {
  ...
  for (let i = 0; i < 4; i++) {
    try {
      ...
      if (response.ok) {
        ...
        // Don't retry on bad requests (4xx) other than 429
        break;
      }
    } catch (error) {
      // Network errors
...
const AdminDashboard = ({ onLogout, t, isDarkMode }) => {
  ...
  const handleFormSubmit = (e) => {
    ...
    setFormState({ trackingNumber: "", name: "", carrier: "", status: "", estimatedDelivery: "" });
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={t('createVirtual')}>
...
export default function App() {
  ...
  const packageHandlers = {
    ...
    handleOrganizeList: async () => {
      setOrganizedCategories(null);
      setGeminiError(null);
      setIsOrganizeLoading(true);
      setIsOrganizeModalOpen(true);
      
      try {
        const packageNames = packages.filter(p => !p.isArchived && p.status !== 'Delivered').map(p => p.name);
        if (packageNames.length === 0) {
            setOrganizedCategories({ "No Upcoming Packages": [] });
        } else {
            const result = await getGeminiOrganization(packageNames);
            setOrganizedCategories(result);
        }
      } catch (error) {
        console.error("Gemini API error:", error);
        setGeminiError(t('geminiError'));
      } finally {
